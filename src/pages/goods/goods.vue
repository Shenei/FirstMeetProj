...
const goodsItem = ref<GoodsResult>()
...
//获取商品信息
const getGoodsDes = async () => {
  let res = await getGoodsByIdAPI(query.id)
  goodsItem.value = res.result
  localdata.value = {
    _id: res.result.id,
...
//预览图
const onTapImage = (pic: string) => {
  uni.previewImage({
    current: pic,
    urls: goodsItem.value!.mainPictures,
  })
}
//切换轮播图的下标改变
const onChange: UniHelper.SwiperOnChange = (e) => {
  activeIdx.value = e.detail.current
}
// uni-ui 弹出层组件 ref
const popup = ref<{
...
<template>
  ...
  <scroll-view scroll-y class="viewport">
    ...
    <view class="goods">
      ...
      <view class="preview">
        <swiper @change="onChange" circular>
          <swiper-item v-for="item in goodsItem?.mainPictures" :key="item">
            <image @tap="onTapImage(item)" mode="aspectFill" :src="item" />
          </swiper-item>
        </swiper>
        <view class="indicator">
          <text class="current">{{ activeIdx + 1 }}</text>
          <text class="split">/</text>
          <text class="total">{{ goodsItem?.mainPictures.length }}</text>
        </view>
      </view>
      ...
      <view class="meta">
        ...
        <view class="desc"> {{ goodsItem?.desc }} </view>
      </view>
    ...
    <view class="detail panel">
      ...
      <view class="content">
        <view class="properties">
          <!-- 属性详情 -->
          <view class="item" v-for="item in goodsItem?.details.properties" :key="item.name">
        ...
        <!-- 图片详情 -->
        <image
          v-for="item in goodsItem?.details.pictures"
          :key="item"
          mode="widthFix"
          :src="item"
        ></image>
    ...
    <view class="similar panel">
      ...
      <view class="content">
        <navigator
          v-for="item in goodsItem?.similarProducts"
          :key="item.id"
          class="goods"
          hover-class="none"
