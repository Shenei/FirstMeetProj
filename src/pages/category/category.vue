...
import { getSwiperData } from '@/services/home'
...
const topCategory = ref<CategoryTopItem[]>([])

const activeIdx = ref(0)
const loading = ref()
const getSwiperPic = async () => {
  let res = await getSwiperData(2)
  swiperData.value = res.result
}
const getCategory = async () => {
  let res = await getCategoryTop()
  topCategory.value = res.result
}
const subCategoryList = computed(() => {
  return topCategory.value[activeIdx.value]?.children || []
})
onLoad(async () => {
  loading.value = true
  await Promise.all([getSwiperPic(), getCategory()])
  loading.value = false
})
</script>
<template>
    ...
    <view class="viewport">
      ...
      <view class="categories">
        ...
        <scroll-view class="secondary" scroll-y>
          <!-- 焦点图 -->
          <RabSwiper class="banner" :list="swiperData" />
          <!-- 内容区域 -->
